...
class SubscriptionController extends Controller
{
    ...
    {   
        ...
        if($request->has('category_name')){
                ...
                ->where('relatedTerms', 'LIKE', "%{$data['sort']}%")
                ->where('type', 'admin_created')
                ->orderBy('id','DESC')
                ->skip($offset)->take($limit)->get();
                
                //  $subscriptions = Subscription::with('favorites')->where('type', 'admin_created')->whereRaw('FIND_IN_SET(?, relatedTerms)', [$data['sort']])->where('type', 'admin_created')->orderBy('id','DESC')->skip($offset)->take($limit)->get();  
